...
describe('utils', () => {
  describe('createClassNames', () => {
    it('expect to return classNames', () => {
      ...
      const expectation = ['cvi-Widget', 'cvi-Widget-one', 'cvi-Widget-two'];

      expect(actual).toEqual(expectation);
    });

    it('expect to return classNames with custom prefix', () => {
      const cx = utils.createClassNames('Widget', 'rvs');

      const actual = cx('', null, undefined, false, 'one', 'two').split(' ');
      const expectation = ['rvs-Widget', 'rvs-Widget-one', 'rvs-Widget-two'];
