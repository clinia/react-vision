...
describe('Pagination', () => {
    ...
    tree = renderer
      .create(
        <Pagination
          {...DEFAULT_PROPS}
          showLast
          currentRefinement={DEFAULT_PROPS.numPages}
        />
      )
      .toJSON();
    expect(tree).toMatchSnapshot();
  ...
  describe('padding behaviour', () => {
    it('should be adjusted when currentPage < padding (at the very beginning)', () => {
      ...
      const pages = wrapper.find('.cvi-Pagination-item--page');
      const pageSelected = wrapper.find('.cvi-Pagination-item--selected');
