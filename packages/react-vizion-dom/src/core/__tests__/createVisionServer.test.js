...
import { findResultsState } from '../createVizionServer';
...
describe('findResultsState', () => {
  ...
  it('adds expected Clinia agents', () => {
    ...
    // - 1 times with @clinia/react-vizion-dom/server
    // - 2 times with @clinia/react-vizion-core/Vizion
    ...
    expect(searchClient.addCliniaAgent).toHaveBeenCalledWith(
      `react-vizion (${version})`
    );
    expect(searchClient.addCliniaAgent).toHaveBeenCalledWith(
      `react-vizion-server (${version})`
    );
  });
